...
class Message {
	...
	public function display($group=FALSE, $wrapper=FALSE){
		...
		$messages = $this->CI->session->flashdata('_messages');
		if ($messages) {
			// Luego eliminar los mensajes flash
			$this->CI->session->set_flashdata('_messages', NULL);
		}
	}
		...
		if(count($this->messages)){
			...
			foreach($output as $type => $messages){
				...
				foreach($messages as $msg){
					$content .= "<i class='icon-remove close' data-dismiss='alert'></i>";
					$content .= ''.$msg.'';
				}
				$content .= '</div>'."\r\n";
			}
