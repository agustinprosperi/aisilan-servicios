...
function uri_current($controller_current, $w, $my_model, $method="index")
{
    $all = ""; $active = ""; $inactive = "";
    ...
    $text .= "<a href='".base_url() . "backend/$controller_current/$method/1' class='$active active-items'>Activos(".count($my_model->getLista('1'))."),</a> ";
    $text .= "<a href='".base_url() . "backend/$controller_current/$method/0' class='$inactive inactive-items'>Inactivos(".count($my_model->getLista('0')).")</a>";
...
function calculoX($inicio, $fin, $prov_id, $loc_id, $horquilla){

    $tipo_hora_madrugada = NULL;
    $tipo_hora_dia = NULL;
    $tipo_hora_noche = NULL;
    ...
    //00:00 -inicio-fin- 06:00
    if(rangoMadrugada($inicio) && rangoMadrugada($fin)){
        $horas_madrugada = $horas_madrugada + calcularHorasEntreFechas($inicio, $fin);
        
        if(verificarFeriado($fin, $prov_id, $loc_id) && $horas_madrugada > 0) {
            $tipo_hora_madrugada = "Festivo noche *";
            $horquilla_madrugada = $horquilla->hor_festivo_nocturno;

        }elseif(!verificarFeriado($fin, $prov_id, $loc_id) && $horas_madrugada > 0){
            $tipo_hora_madrugada = "Laborable noche *";
            $horquilla_madrugada = $horquilla->hor_laborable_nocturno;
        }
    }

    //00:00 -inicio- 06:00 ------  06:00 -fin- 22:00
    elseif(rangoMadrugada($inicio) && rangoDia($fin)){
        $fecha_madrugada = date("Y-m-d", strtotime($inicio));
        $horas_madrugada = $horas_madrugada + calcularHorasEntreFechas($inicio, $fecha_madrugada." 06:00:00");

        $fecha_dia = date("Y-m-d", strtotime($fin));
        $horas_dia = $horas_dia + calcularHorasEntreFechas($fecha_dia." 06:00:00", $fin);

        if(verificarFeriado($fin, $prov_id, $loc_id) && $horas_dia > 0) {
            $tipo_hora_dia = "Festivo día";
            $horquilla_dia = $horquilla->hor_festivo; 
            
        }elseif(!verificarFeriado($fin, $prov_id, $loc_id) && $horas_dia > 0){
            $tipo_hora_dia = "Laborable día";
            $horquilla_dia = $horquilla->hor_laborable;
        }       
        
        if(verificarFeriado($inicio, $prov_id, $loc_id) && $horas_madrugada > 0) {
            $tipo_hora_madrugada = "Festivo noche *";
            $horquilla_madrugada = $horquilla->hor_festivo_nocturno;

        }elseif(!verificarFeriado($inicio, $prov_id, $loc_id) && $horas_madrugada > 0){
            $tipo_hora_madrugada = "Laborable noche *";
            $horquilla_madrugada = $horquilla->hor_laborable_nocturno;
        }
    }
    //06:00 -inicio-fin- 22:00
    elseif(rangoDia($inicio) && rangoDia($fin)){
        $horas_dia = $horas_dia + calcularHorasEntreFechas($inicio, $fin);

        if(verificarFeriado($inicio, $prov_id, $loc_id) && $horas_dia > 0) {
            $tipo_hora_dia = "Festivo día";
            $horquilla_dia = $horquilla->hor_festivo; 
            
        }elseif(!verificarFeriado($inicio, $prov_id, $loc_id) && $horas_dia > 0){
            $tipo_hora_dia = "Laborable día";
            $horquilla_dia = $horquilla->hor_laborable;
        } 
    } 
    
    //06:00 -inicio-22:00 ----- 22:00 -fin- 00:00
    elseif(rangoDia($inicio) && rangoNoche($fin)){
        $fecha_dia = date("Y-m-d", strtotime($inicio));
        $horas_dia = $horas_dia + calcularHorasEntreFechas($inicio, $fecha_dia." 22:00:00");

        $fecha_noche = date("Y-m-d", strtotime($fin));
        $horas_noche = $horas_noche + calcularHorasEntreFechas($fecha_noche." 22:00:00", $fin);
        ...
        if(verificarFeriado($fin, $prov_id, $loc_id) && $horas_noche > 0) {
            $tipo_hora_noche = "Festivo noche";
            $horquilla_noche = $horquilla->hor_festivo_nocturno;
        }elseif(!verificarFeriado($fin, $prov_id, $loc_id) && $horas_noche > 0){
            $tipo_hora_noche = "Laborable noche";
            $horquilla_noche = $horquilla->hor_laborable_nocturno;
        }
    }
    //22:00 -inicio-fin- 00:00
    elseif(rangoNoche($inicio) && rangoNoche($fin)){
        $horas_noche = $horas_noche + calcularHorasEntreFechas($inicio, $fin);

        if(verificarFeriado($inicio, $prov_id, $loc_id) && $horas_noche > 0) {
            $tipo_hora_noche = "Festivo noche";
            $horquilla_noche = $horquilla->hor_festivo_nocturno;
        }elseif(!verificarFeriado($inicio, $prov_id, $loc_id) && $horas_noche > 0){
            $tipo_hora_noche = "Laborable noche";
            $horquilla_noche = $horquilla->hor_laborable_nocturno;
        } 
    }
    //06:00 -inicio- 22:00 ----- +2 noche ----- 00:00 -fin- 06:00
    elseif(rangoDia($inicio) && rangoMadrugada($fin)){
        $fecha_dia = date("Y-m-d", strtotime($inicio));
        $horas_dia = $horas_dia + calcularHorasEntreFechas($inicio, $fecha_dia." 22:00:00");

        $horas_noche = 2;

        $fecha_madrugada = date("Y-m-d", strtotime($fin));
        $horas_madrugada = $horas_madrugada + calcularHorasEntreFechas($fecha_madrugada." 00:00:00", $fin);
    ...
    //00:00 -inicio- 06:00 ----- +12 dia ----- 22:00 -fin- 00:00
    elseif(rangoMadrugada($inicio) && rangoNoche($fin)){
        $fecha_madrugada = date("Y-m-d", strtotime($inicio));/////////////////////////*****************************************ME PARECE QUE DA ERROR, FECHA MADRUGADA DEBE ESTAR CON INICIO */
                                                //AQui cambie $fin por $inicio, aver si no se rompe  
        $horas_madrugada = $horas_madrugada + calcularHorasEntreFechas($inicio, $fecha_madrugada.' 06:00:00');
        ...
        if(verificarFeriado($inicio, $prov_id, $loc_id) && $horas_dia > 0) {
            $tipo_hora_dia = "Festivo dia";
            $horquilla_dia = $horquilla->hor_festivo;
        }elseif(!verificarFeriado($inicio, $prov_id, $loc_id) && $horas_dia > 0){
            $tipo_hora_dia = "Laborable dia";
            $horquilla_dia = $horquilla->hor_laborable;
        } 
    ...
    //22:00 -inicio- 00:00 ----- 00:00 -fin- 06:00
    elseif(rangoNoche($inicio) && rangoMadrugada($fin)){

        //22:00 to 00:00
        $fecha_noche = date("Y-m-d", strtotime($fin));
        $horas_noche = $horas_noche + calcularHorasEntreFechas($inicio, $fecha_noche." 00:00:00");
    ...
    return array(
        "hora_inicio" => date("H:i", strtotime($inicio)),
        "hora_fin"    => date("H:i", strtotime($fin)),

        "hora_madrugada" => $horas_madrugada,
        "hora_dia" => $horas_dia, 
        "hora_noche" => $horas_noche,
            "hora_noche_madrugada" => ($horas_noche + $horas_madrugada),

        "tipo_hora_madrugada" => $tipo_hora_madrugada,
        "tipo_hora_dia" => $tipo_hora_dia,
        "tipo_hora_noche" => $tipo_hora_noche,

        "horquilla_madrugada" => $horquilla_madrugada,
...
function verificarFeriado___X($prov_id, $loc_id, $fecha_inicio, $fecha_fin){
    
    $feriado_domingo___fecha_inicio = date('w', strtotime($fecha_inicio))==0?true:false;
    $feriado_domingo___fecha_fin = date('w', strtotime($fecha_fin))==0?true:false;
    
    $fecha_inicio = date("Y-m-d", strtotime($fecha_inicio));
    $fecha_fin = date("Y-m-d", strtotime($fecha_fin));
    
    $feriado_local___fecha_inicio = $ci->feriado_modelo->getFeriadoLocal($prov_id, $loc_id, $fecha_inicio);
    $feriado_local___fecha_fin = $ci->feriado_modelo->getFeriadoLocal($prov_id, $loc_id, $fecha_fin);

    $feriado_nacional___fecha_inicio = $ci->feriado_modelo->getFeriadoNacional($fecha_inicio);
    $feriado_nacional___fecha_fin = $ci->feriado_modelo->getFeriadoNacional($fecha_fin);

    //si cualquiera de las 6 viarables de arriba da true entonces es feriado
    //if($feriado_domingo___fecha_inicio or $feriado_domingo___fecha_fin)
}

function verificarFeriado($fecha, $prov_id=0, $loc_id=0){
    $ci = &get_instance();
    $ci->load->model("feriado_modelo");

    $feriado_domingo___fecha = date('w', strtotime($fecha))==0?true:false;
    
    $fecha = date("Y-m-d", strtotime($fecha));
    
    $feriado_local___fecha = $ci->feriado_modelo->getFeriadoLocal($fecha, $prov_id, $loc_id);

    $feriado_nacional___fecha = $ci->feriado_modelo->getFeriadoNacional($fecha);

    //si cualquiera de las 6 viarables de arriba da true entonces es feriado
    if($feriado_domingo___fecha or $feriado_local___fecha or $feriado_nacional___fecha)
        return true;
    else
        return false;
}
...
/**
 * Functión para cambiar nueva fecha a otras 2 fechas to y from
 */
