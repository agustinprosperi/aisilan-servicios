...
if ($installed == true) {
	...
	if (empty($_REQUEST['ERROR'])) {
		if ((float)$_SETTINGS['SERVERVERSION'] >= 3.80) { // iPhone PUSH Supported
			$query = sprintf("SELECT `id`, `department`, `device` FROM " . $table_prefix . "users WHERE (`refresh` > DATE_SUB(NOW(), INTERVAL %d SECOND) AND `status` = '1') OR (`device` <> '' AND `status` = '1')", $connection_timeout);
		} else {
...
if ($_SETTINGS['DEPARTMENTS'] == true && !isset($_REQUEST['DEPARTMENT']) && $installed == true || $_REQUEST['ERROR'] == 'empty')  {
	...
	if ((float)$_SETTINGS['SERVERVERSION'] >= 3.80) { // iPhone PUSH Supported
		$query = sprintf("SELECT DISTINCT `department` FROM " . $table_prefix . "users WHERE (`refresh` > DATE_SUB(NOW(), INTERVAL %d SECOND) AND `status` = '1') OR (`device` <> '' AND `status` = '1') ORDER BY `department`", $connection_timeout);
	} else {
