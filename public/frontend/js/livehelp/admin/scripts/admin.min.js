...
if(session.length>0){var a=service+"?Visitors",c="Session="+session+"&Acción=&Request=&Record=0&Total=25&Version=4.0&Format=json";$.ajax({url:a,data:c,type:"POST",success:function(g){if(g.Visitors!==null&&g.Visitors!==undefined&&g.Visitors.Visitor!==undefined){visitors=g.Visitors.Visitor;
...
var y=v.data("scroll");if(u>0){y=v.find(".scroll .end");}var z=$(".chatting .visitor"),B=$(".chat-stack .dropdown-menu.options"),A=v.data("id");$.each(z,function(D,E){if($(E).data("id")===A){var C=$('.chatting .visitor[data-id="'+A+'"]').data("user").Email,F=B.find(".EmailChatVisitor");
F.text("Visitor's Email Address ("+C+")").data("email",C);F.data("id",A);return;}});checkBlocked(A);if(y!==undefined){v.find(".scroll").scrollTo(y);}});
$(".chat-stack .smilies.button").close();v.find(".name").hide();var s=v.find(".message-alert");$(s).data("total",0).hide();$(".chat-stack .chat").each(function(C,D){var B=$(D),F=parseInt(B.css("left"),10),y=parseInt(B.css("z-index"),10),E=0,A=2,z="#fafafa";
...
},dataType:"json"});}}k.find(".save.button").click(function(){h();});function p(r){if(r!==undefined&&r.length>0){$.each(r,function(t,s){k.find(".add-response.tags").append('<span class="tag"><span class="tag-icon"></span>'+s.trim()+'<span class="delete" title="Eliminar"></span></span>');
...
});}var historyGrid,historyColumns;function initHistoryGrid(d){function b(y,w,t,r,h){var p="Chrome",j="",u=h.Country;t=h.UserAgent;if(t.indexOf("MSIE")!==-1){p="InternetExplorer";
}else{if(t.indexOf("Chrome")!==-1){p="Chrome";}else{if(t.indexOf("Opera")!==-1){p="Opera";}else{if(t.indexOf("Safari")!==-1){p="Safari";}else{if(t.indexOf("Firefox")!==-1){p="Firefox";
}}}}}h.Username=ucwords(h.Username.toLowerCase());h.UserAgent="./images/"+p+".png";if(h.State!==null&&h.State.length>0){if(h.City!==null&&h.City.length>0){u=h.City+", "+h.State+", "+h.Country;
}else{u=h.State+", "+h.Country;}}else{if(h.City!==null&&h.City.length>0){u=h.City+", "+h.Country;}}j='<span class="'+convertCountryIcon(h.Country)+'" style="float:left; margin: 3px 5px 3px 0; display:inline-block"></span>';
h.Location=j+u;if(h.Department.length===0){h.Department="Unavailable";}if(h.Email.length>0){if(!$(h.Email).text().length){h.Email='<a href="mailto:'+h.Email+'">'+h.Email+"</a>";
}}else{h.Email="Unavailable";}if(!$(h.CurrentPage).text().length){h.CurrentPage='<a href="'+h.CurrentPage+'" target="_blank">'+h.CurrentPage+"</a>";}var s=h.Referrer,x=/^http[s]{0,1}:\/\/(?:[^.]+[\\.])*google(?:(?:.[a-z]{2,3}){1,2})[\/](?:search|url|imgres)(?:\?|.*&)q=([^&]*)/i,o=x.exec(s);
if(o!==null){if(o[1].length>0){s="Google Search (Keywords: "+o[1]+")";}else{s="Google Search";}}h.Referrer=s;var k=parseInt(h.Rating,10),v="No Rating",n="";
switch(k){case 1:v="Very Poor";break;case 2:v="Poor";break;case 3:v="Good";break;case 4:v="Very Good";break;case 5:v="Excellent";break;}for(var m=1;m<=5;
m++){var q="",l="";if(k>=m){q="-highlight";}if(k===0){l=" rating-disabled";}n+='<div class="rating'+q+l+'"></div>';}n+='<span style="margin-left:5px">'+v+"</span>";
h.Rating=n;var w='<div class="cell-inner"> 	<div class="cell-left"> 		<img src="'+h.UserAgent+'"/> 	</div> 	<div class="cell-main"> 	  <h2 style="text-transform: none">'+h.Username+"</h2> 	  <span>"+h.Operator+"</span><br/> 	  <span>"+h.Department+"</span><br/> 	  <span>"+h.Hostname+"</span><br/> 	  <span>"+h.Email+"</span><br/> 	  <span>"+h.Referrer+"</span><br/> 	  <span>"+h.CurrentPage+"</span><br/> 	  <span>"+h.Location+"</span><br/> 	  <span>"+h.Rating+"</span><br/> 	</div> </div>";
return w;}var g,a={rowHeight:220,enableCellNavigation:true,enableColumnReorder:false,multiColumnSort:true,multiSelect:false};if(d===undefined){d=$.jStorage.get("history-date","");
var f=$(".history #calendar");f.find("td").removeClass("selected-date");f.find('td[id="'+d+'"]').addClass("selected-date");d=(d.length>0)?new Date.parse(d):new Date();
d=d.getFullYear()+"-"+zeroFill(d.getMonth()+1,2)+"-"+zeroFill(d.getDate(),2);}if(session.length>0){var c=service+"?History",e="Session="+session+"&StartDate="+d+"&EndDate="+d+"&Timezone="+new Date().getUTCOffset()+"&Transcripts=1&ID=0&Version=4.0&Format=json";
$.ajax({url:c,data:e,type:"POST",success:function(j){if(j.ChatHistory!==undefined){g=[];$.each(j.ChatHistory,function(l,m){g.push(m.Visitor);});g=g.reverse();
...
historyColumns[0].formatter=b;historyColumns[1].formatter=Slick.Formatters.Date;}historyGrid=new Slick.Grid(".history-grid",g,historyColumns,a);historyGrid.setSelectionModel(new Slick.RowSelectionModel());
...
chat=g[rows];if(chat!==undefined){openHistoryChat(chat);}});historyGrid.onSort.subscribe(function(n,l){var m=l.sortCols;g.sort(function(v,u){for(var q=0,p=m.length;
q<p;q++){var t=m[q].sortCol.field;var o=m[q].sortAsc?1:-1;var s=v[t],r=u[t];var w=(s==r?0:(s>r?1:-1))*o;if(w!==0){return w;}}return 0;});historyGrid.invalidate();
historyGrid.render();});var k="",h=g.length;if(h>1){k=" - "+g.length+" chats";}else{if(h>0){k=" - "+g.length+" chat";}}if(h>0){$(".history-empty").hide();
$(".history-grid").show();}else{$(".history-grid").hide();$(".history-empty").show();}$(".username-column-header .slick-column-name").text("Chat History"+k);
...
var d=lastUpdatedAccount(c);if(d>b||a){updateAccountsGrid(c);}}var accountsGrid,accountsDataView,accounts,columns=[{id:"account",name:"",field:"Username",formatter:renderAccountCell,sortable:true}],options={rowHeight:80,enableCellNavigation:true,enableColumnReorder:false,forceFitColumns:true,multiColumnSort:true,multiSelect:false},accountTemplate=null;
function renderAccountCell(g,b,f,d,a){var e=a,c=parseInt(e.Status,10);if(!isNaN(c)){switch(c){case 0:c="Offline";break;case 1:c="Online";break;case 2:c="Be Right Back";
break;case 3:c="Away";break;}e.Status=c;}e.Mobile="none";if(e.Device.length>0){e.Mobile="inline-block";}e.Image="../image.php?ID="+e.ID+"&SIZE=100";var b='<div class="cell-inner" data-id="'+e.ID+'"> <div class="cell-left"> 	<img src="'+e.Image+'"/> </div> <div class="cell-main"> 	<div class="cell-heading name">'+e.Firstname+" "+e.Lastname+'</div> 	<span class="cell-details department">'+e.Department+'</span> 	<span class="cell-details status">'+e.Status+'</span> 	<span class="disabled" title="Account Disabled" style="width:22px; height:22px; display:inline-block; position:absolute; bottom:10px; right:30px; opacity:0.3; display:'+e.Disabled+'"></span> 	<span class="sprite Smartphone" title="Logged in with Mobile App" style="position:absolute; bottom:10px; right:25px; opacity:0.5; display:'+e.Mobile+'"></span> </div> </div>';
return b;}function updateAccountsGrid(a){accountsDataView=new Slick.Data.DataView();accountsGrid=new Slick.Grid(".accounts-grid",accountsDataView,columns,options);
accountsGrid.setSelectionModel(new Slick.RowSelectionModel());accountsGrid.onSelectedRowsChanged.subscribe(function(){rows=accountsGrid.getSelectedRows();
account=accountsDataView.getItem(rows);if(account!==undefined){var b=$.jStorage.get("accounts",[]);$.each(b,function(c,d){if(parseInt(account.ID,10)===parseInt(d.ID,10)){account=d;
}});showAccount(account);accountsGrid.setSelectedRows([]);accountsGrid.resetActiveCell();}});accountsGrid.onSort.subscribe(function(d,b){var c=b.sortCols;
accountsDataView.sort(function(n,m){for(var g=0,f=c.length;g<f;g++){var k=c[g].sortCol.field;var e=c[g].sortAsc?1:-1;var j=n[k],h=m[k];var o=(j==h?0:(j>h?1:-1))*e;
if(o!==0){return o;}}return 0;});accountsGrid.invalidate();accountsGrid.render();});accountsDataView.onRowCountChanged.subscribe(function(c,b){accountsGrid.updateRowCount();
accountsGrid.render();});accountsDataView.onRowsChanged.subscribe(function(c,b){accountsGrid.invalidateRows(b.rows);accountsGrid.render();});accountsDataView.beginUpdate();
accountsDataView.setItems(a);accountsDataView.endUpdate();}function initAccountsGrid(g){if(session.length>0){var b=service+"?Operators",d=new Date().toString("yyyy-MM-dd HH:mm:ss"),c="Session="+session+"&Cached="+d+"&Version=4.0&Format=json",f=$.jStorage.get("accounts",[]),a=lastUpdatedAccount(f);
...
b.animate({width:0,opacity:0},250,function(){b.hide();});switchPreviousMenu();}var notifications=[];function updateUser(l,c){var h="",m=false,o=false.hash="",f=encodeURIComponent(window.location.protocol+"//"+window.location.host+window.location.pathname.replace(window.location.pathname.substring(window.location.pathname.indexOf("/livehelp/admin/")),"/livehelp/images/UserSmall.png")),a=$(".chatting.list"),j=$(".other-chatting.list"),b=$(".pending.list"),e=$(".operators.list"),g=null,n=0,k=false,d=$("#chatstotal");
...
if(c!==undefined){$.extend(post,{Acción:c});}if(d!==undefined){$.extend(post,{ID:d});}window.clearTimeout(usersTimer);$.ajax({url:b,data:post,type:"POST",success:function(e){updateUsersInterface(e,c,d);
...
}}var session="",factor="",error=false;function signInComplete(){$(".login").fadeOut();$(".content").animate({opacity:1},250);if(messageSound===undefined){messageSound=new buzz.sound("../sounds/New Message",{formats:["ogg","mp3","wav"],volume:100});
}if(pendingSound===undefined){pendingSound=new buzz.sound("../sounds/Pending Chat",{formats:["ogg","mp3","wav"],volume:100});}operator=$.jStorage.get("operator");
...
if(b.length>0&&g.length>0){$.extend(e,{Username:b,Password:g,Acción:a});}else{var f=$.jStorage.get("session","");if(f.length>0){$.extend(e,{Session:f});
