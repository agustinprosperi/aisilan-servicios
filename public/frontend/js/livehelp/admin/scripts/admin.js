...
function updateVisitorsGrid() {
	...
	if (session.length > 0) {
		...
		var url = service + '?Visitors',
			post = 'Session=' + session + '&Acción=&Request=&Record=0&Total=25&Version=4.0&Format=json';
	...
	function addTags(tags) {
		if (tags !== undefined && tags.length > 0) {
			$.each(tags, function (key, tag) {
				responsesParent.find('.add-response.tags').append('<span class="tag"><span class="tag-icon"></span>' + tag.trim() + '<span class="delete" title="Eliminar"></span></span>');
			});
		}
	}
...
function initHistoryGrid(date) {
	...
	<div class="cell-main"> \
	  ...
	  <span>' + dataContext.Rating + '</span><br/> \
	</div> \
</div>';
...
function updateAccountsGrid(accounts) {
	...
	accountsGrid = new Slick.Grid('.accounts-grid', accountsDataView, columns, options);
...
function updateUser(section, users) {
	var html = '',
		...
		staff = false.
		hash = '',
		defaultimage = encodeURIComponent(window.location.protocol + '//' + window.location.host + window.location.pathname.replace(window.location.pathname.substring(window.location.pathname.indexOf('/livehelp/admin/')), '/livehelp/images/UserSmall.png')),
...
function signIn() {
	...
	if (user.length > 0 && pass.length > 0) {
		$.extend(post, {'Username': user, 'Password': pass, 'Acción': status});
	} else {
		// Sign In / Saved Session
		var autoLogin = $.jStorage.get('session', '');
		if (autoLogin.length > 0) {
