...
if ($_REQUEST['txid'] != '' && isset($_SETTINGS['ANALYTICS'])) {
	...
	$query = sprintf("INSERT INTO " . $table_prefix . "orders(`order`, `request`, `datetime`, `affiliation`, `total`, `tax`, `shipping`, `city`, `state`, `country`) VALUES('%s', '%s', NOW(), '%s', '%s', '%s', '%s', '%s', '%s', '%s')", $SQL->escape($_REQUEST['txid']), $SQL->escape($request), $SQL->escape($_REQUEST['affil']), $SQL->escape($_REQUEST['total']), $SQL->escape($_REQUEST['tax']), $SQL->escape($_REQUEST['ship']), $SQL->escape($_REQUEST['city']), $SQL->escape($_REQUEST['state']), $SQL->escape($_REQUEST['country']));
	...
	if ($order != false) {
		...
		foreach ($_REQUEST['im'] as $key => $item) {
			...
			$query = sprintf("SELECT `id` FROM " . $table_prefix . "products WHERE `code` = '%s' LIMIT 1", $SQL->escape($item['sku']));
			$product = $SQL->selectquery($query);
			if ($product == false) {
				$query = sprintf("INSERT INTO " . $table_prefix . "products(`code`, `name`) VALUES ('%s', '%s')", $SQL->escape($item['sku']), $SQL->escape($item['name']));
				$product = $SQL->insertquery($query);
			}
			
			$item['price'] = validatePrice($item['price']);
			if ($count == 0) {
				$items = sprintf("INSERT INTO " . $table_prefix . "orderproducts(`order`, `product`, `category`, `price`, `quantity`) VALUES ('%s', '%s', '%s', '%s', '%s')", $SQL->escape($order), $SQL->escape($product), $SQL->escape($item['category']), $SQL->escape($item['price']), $SQL->escape($item['qty'])); 
			} elseif ($count > 0) {
